...
<script type="text/javascript">
    ...
    <div class="rightpanel">
        ...
        <div class="pagetitle">
            <h1>车辆列表</h1> <span>用来显示所有车辆相关信息</span>
        </div><!--pagetitle-->
        
        <div class="maincontent">
            <div class="contentinner">
            
                <h4 class="widgettitle">分类列表</h4>
                <table class="table table-bordered" id="dyntable">
                    ...
                    <tbody>
                        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="gradeX">
                                ...
                                <td class="center">
                                        <?php if(($vo["yprice"] == '')): ?>无优惠 
                                        <?php elseif(($vo["start_time"] > time())): ?>未开始
                                        <?php elseif(($vo["end_time"] < time())): ?>已过期                     
                                        <?php else: echo ($vo["yprice"]); endif; ?>
                                </td>
                                <td class="center"><?php echo (date("Y-m-d H:i:s",$vo["regtime"])); ?></td>
                                <td class="center"><?php echo ($vo['status']==1?在售:下架); ?></td>
                                <td class="center">
                                    <a href="__APP__/Foods/upFoods/id/<?php echo ($vo["id"]); ?>" title="修改">
                                        <span class="icon-edit"></span>
                                    </a>
                                </td>
                                <td class="center">
                                    <a href="__APP__/Foods/del/id/<?php echo ($vo["id"]); ?>" onclick="return confirm('你确定要删除吗?');" title="删除">
